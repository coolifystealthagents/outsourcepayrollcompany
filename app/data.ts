...
import { september1BlogDetails, september1BlogPosts } from './september1-content';
import { september2BlogDetails, september2BlogPosts } from './september2-content';
import { september3BlogDetails, september3BlogPosts } from './september3-blog';
...
import { september10BlogDetails, september10BlogPosts } from './september10-content';
import { september11BlogDetails, september11BlogPosts } from './september11-content';
import { september14BlogDetails, september14BlogPosts } from './september14-content';
import { september18BlogDetails, september18BlogPosts } from './september18-blog';
import { september22BlogDetails, september22BlogPosts } from './september22-blog';
...
export const site = {
  ...
  color: '#15803d',
  accent: '#0ea5e9',
  heroImage: '/payroll-operations-team.jpg',
  serviceImage: '/payroll-operations-team.jpg',
  alt: 'Coworkers reviewing notes together at a shared table',
  badge: 'Payroll ops',
} as const;
...
export const services = [
  {
    ...
    desc: 'Filipino payroll operations help for timesheet chasing, employee-change logs, pay-period checklists, and payroll-provider handoffs.',
    ...
    controls: [
      'the Filipino payroll assistant prepares payroll data but does not approve pay, change bank details alone, or submit final payroll',
    ...
    faqs: [
      { question: 'Can a Filipino payroll assistant run payroll?', answer: 'No. Treat the role as payroll prep and follow-up support. A payroll owner or provider should approve final pay, tax, benefits, and banking actions.' },
  ...
  },
  {
    ...
    faqs: [
      { question: 'Can a Filipino payroll assistant answer employee pay questions?', answer: 'They can send approved status replies and collect details. Pay amount, tax, garnishment, and policy answers should come from the payroll owner.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { question: 'What makes this work a good fit for Filipino talent?', answer: 'The work repeats each pay period, has clear examples, and can be checked with a simple list.' },
      { question: 'How much access should we give first?', answer: 'Start with the least access needed. Add permissions only after the Filipino payroll assistant shows they can follow the process.' },
    ],
  },
  {
    ...
    launchPlan: [
      ...
      'have the Filipino payroll assistant prepare the report before cut-off while the owner reviews the exceptions',
    ...
    faqs: [
      ...
      { question: 'How do we know the Filipino payroll assistant is helping?', answer: 'Track fewer late edits, fewer manager chases, cleaner handoff notes, and faster payroll-owner review.' },
    ],
  },
] as const;
...
export const blogDetails: Record<string, BlogDetail> = {
  ...
  'outsource-payroll-company-planning': {
    takeaways: [
      ...
      'Give each Filipino payroll worker only the system access needed for the assigned task.',
    ...
    sections: [
      {
        ...
        paragraphs: [
          'Payroll outsourcing can mean two different things. A payroll company may calculate wages, taxes, and checks. A Filipino payroll support worker may collect the inputs, chase missing approvals, keep the change log clean, and prepare the handoff. Decide which kind of help you need before you compare providers.',
      ...
      },
      {
        ...
        paragraphs: [
          'A vague job description is risky around payroll data. Write down what the Filipino payroll support worker can prepare, what they can enter, and what they must stop and send to an owner. Use plain examples. For instance: "You may add an approved address change to the tracker. You may not change a bank account or pay rate without a second approval."',
          'Name the person who gives that approval. A shared inbox is not an owner. The Filipino payroll worker needs a person, a backup person, and a response time for urgent items near payroll cut-off. This keeps a late question from turning into a guess.',
        ],
        bullets: [
          'The Filipino payroll worker may collect and organize source records.',
          'The Filipino payroll worker may flag missing data and send approved reminders.',
    ...
    roleBrief: [
      ...
      'Tools: List the payroll, timekeeping, HR, file, and ticket systems the Filipino payroll worker may use',
    ...
    faqs: [
      ...
      { question: 'Can a Philippines-based support worker approve payroll?', answer: 'Keep final approval with your authorized payroll owner or provider. A Filipino payroll support worker can prepare records and flag exceptions, but should not decide disputed pay, taxes, bank changes, or deductions.' },
      { question: 'How much system access should the Filipino payroll worker get?', answer: 'Give the least access needed for the first task lane. Use a named account and MFA. Add permissions only when a real task requires them and an owner agrees.' },
