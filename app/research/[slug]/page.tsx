...
export function generateMetadata({params}:{params:Promise<{slug:string}>}){return params.then(({slug})=>{const p=researchPosts.find(x=>x.slug===slug);const canonical=`${siteUrl}/research/${slug}`;const modified=p?.updated??p?.published;return p?{title:p.title,description:p.excerpt,alternates:{canonical},openGraph:{title:p.title,description:p.excerpt,url:canonical,siteName:'Outsource Payroll Company',type:'article',modifiedTime:modified,images:p.image?[{url:`${siteUrl}${p.image}`}]:undefined}}:{}})}
...
export default async function ResearchArticle({params}:{params:Promise<{slug:string}>}){
  ...
  const schema={'@context':'https://schema.org','@type':'Article',headline:post.title,datePublished:post.published,dateModified:post.updated??post.published,description:post.excerpt,author:{'@type':'Organization',name:'Outsource Payroll Company'},mainEntityOfPage:`https://outsourcepayrollcompany.com/research/${post.slug}`};
  return <><Header/><main className="fleet-main"><article className="section article-shell publisher-article">
    <p className="eyebrow">Research brief · Published: <time dateTime={post.published}>{formatPublicationDate(post.published)}</time>{post.updated&&post.updated!==post.published&&<> · Updated <time dateTime={post.updated}>{formatPublicationDate(post.updated)}</time></>}</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p>{post.image&&<img className="article-hero-image" src={post.image} alt="" width={1024} height={1024} />}
