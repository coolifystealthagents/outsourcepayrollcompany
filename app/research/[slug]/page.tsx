...
export default async function ResearchArticle({params}:{params:Promise<{slug:string}>}){
  ...
  return <><Header/><main className="fleet-main"><article className="section article-shell publisher-article">
    <p className="eyebrow">Research brief · <time dateTime={post.published}>{formatPublicationDate(post.published)}</time></p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p>{post.image&&<img className="article-hero-image" src={post.image} alt="" width={1024} height={1024} />}
